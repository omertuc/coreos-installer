...
fn write_partition_mapping(
    ...
    if objlen < extent.length {
        n += copy_exactly_n(&mut object, w, objlen, buf).with_context(|| format!("aa {:?}", object))?;
        n += copy_exactly_n(&mut io::repeat(0), w, extent.length - objlen, buf).with_context(|| format!("bb {:?}", object))?;
    } else {
        n += copy_exactly_n(&mut object, w, extent.length, buf).with_context(|| format!("aaa {:?}", object))?;
    }
